...
+ $\Omega_b$ - the demographic matrix for blood feeding mosquitoes:  
$$\Omega_b = \mbox{diag}\left(g\right) - \mbox{diag}\left(\sigma_b\right) \left(I - \cal K_b \right)$$

+ $\Omega_q$ - the demographic matrix for egg laying mosquitoes: $$\mbox{diag} \left(g\right) - \mbox{diag} \left(\sigma_q\right) \left(I - \cal K_q \right)$$
...
The `xde_setup()` utilities allow the user to pass a single version of the dispersal matrix $\cal K.$ After using `xde_setup()`
